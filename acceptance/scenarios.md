...
2. Dealcraft reads it and says: `listed $52 · suggested $41 · priced out 3 of 14`
3. Three of my fourteen customers go quiet at $52, so they never text me
   about it at all.
4. At $41 nobody is priced out.

This is the scenario I would never have found by playing. Over-pricing does not
produce refusals — it produces silence, and this is the only place that
silence becomes visible.

**It used to suggest $48 and name $41 separately, as the price that still
cleared everybody.** Reading the binary settled that the two are one number:
what a customer pays is `scalar × price × quantity` while the quantity is
`scalar × budget ÷ price`, so the price cancels out of the money entirely and
only decides whether they order at all. A higher price therefore never earns
more — it only loses customers. The suggestion is the highest price at which
nobody goes quiet.
...
**Rewritten. The old version of this scenario was false**, and it was disproved
while it sat in the file the owner was being asked to approve. It claimed a
handover Dealcraft completed would pay a guest and show on his screen. The
opposite holds: cash is local, so the payment lands wherever the handover runs, the completion popup is
