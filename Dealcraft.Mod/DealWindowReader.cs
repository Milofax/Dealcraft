...
internal sealed class DealWindowReader
{
    ...
    public string Shape()
    {
        try
        {
            // The clock rides with the shape, because the shape alone has never
            // settled an argument and the clock has settled several. Every deal
            // is placed by counting from the window the clock is in, and that
            // window was being reported only in the sentence that names it —
            // never the time it was read from. The owner and the mod disagreed
            // about the time of day for an hour with no way to tell who was
            // right.
            return $"{DealWindowInfo.WINDOW_COUNT} windows of "
                + $"{DealWindowInfo.WINDOW_DURATION_MINS} minutes each, and it is now "
                + $"{Clock()}";
        }
        catch (Exception error)
        {
    ...
    /// <summary>
    /// The clock and the window it falls inside, as one phrase for the log.
    /// </summary>
    private string Clock()
    {
        try
        {
            ...
            {
                return "a time the game has not started keeping yet";
            }
            ...
            int clock = time.CurrentTime;

            return $"{Spell(clock)}, which is {Name(Containing(clock) ?? WindowOf(clock))}";
        }
        catch (Exception error)
        {
            return $"a time that could not be read ({error.Message})";
        }
    }
    ...
    public DealWindow? CurrentWindow()
    {
        try
        {
            ...
            int clock = time.CurrentTime;
            DealWindow? theirs = WindowOf(clock);
            DealWindow? ours = Containing(clock);

            // Said once per pair, so a disagreement is loud and an agreement is
            // silent. The owner met the disagreement before this line existed:
            // on 2026-09-19 three deals taken in the afternoon were all recorded
            // as "Afternoon is the next window you allow today", which is what
            // the walk says when it believes the clock is still in the morning.
            if (theirs != ours)
            {
                Complain(
                    $"the clock reads {Spell(clock)} and the window it falls inside is "
                    + $"{Name(ours)}, but the game's own lookup answers {Name(theirs)} — "
                    + "the window the deal is counted from is the first of those");
            }

            // Ours, and deliberately: GetWindow takes a window's start time, and
            // a running clock is not one. Reading the window a time falls inside
            // is a different question from looking a start time up, and it is
            // the question the scheduler is asking.
            return ours ?? theirs;
        }
        catch (Exception error)
        {
    ...
    /// <summary>
    /// The window a clock time falls inside, from the bounds the game reports
    /// for its own windows.
    /// </summary>
    /// <remarks>
    /// No hour is written down here either: every bound comes from
    /// <c>DealWindowInfo</c>, so a patch that moves a window moves this with it.
    /// The wrap matters — Late Night runs from midnight to six and is the one
    /// window whose start is not before its end in the day's order.
    /// </remarks>
    private DealWindow? Containing(int clock)
    {
        try
        {
            foreach (DealWindowHours hours in Hours())
            {
                int from = hours.StartTime;
                int to = hours.EndTime;
        ...
        {
            // A window the game could not be asked about is not an answer, and
            // the lookup above still has one.
        }

        return null;
    }

    private static string Name(DealWindow? window) =>
        window is null ? "nothing" : DealWindowName.Of(window.Value);
