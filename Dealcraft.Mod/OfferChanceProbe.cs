...
internal sealed class OfferChanceProbe
{
    ...
    /// <summary>
    /// Put the product in something. An instance straight out of
    /// <c>GetDefaultInstance</c> carries no packaging, and the game will not
    /// price one.
    /// </summary>
    /// <remarks>
    /// <para>
    /// <b>This is what made the headline feature answer zero for four
    /// sessions.</b> On 2026-09-18 the probe's own record finally said it:
    /// <i>"handed in 1 × Trio mit 4 Fäusten at 1 per package, quality Standard
    /// (2), no packaging applied; 14 price points, first 1 for $1000000 answered
    /// 0, last 1 for $0 answered 0, best anywhere 0; nothing threw"</i>. A
    /// customer refusing a free product is not a customer refusing a price. The
    /// offer was not one.
    /// </para>
    /// <para>
    /// <c>Customer.GetOfferSuccessChance</c> (RVA <c>0x6A8EE0</c>) calls
    /// <c>ProductItemInstance.get_AppliedPackaging()</c> and
    /// <c>UnityEngine.Object.op_Equality</c> next to each other, then reads
    /// <c>get_Definition()</c>, <c>GetProductEnjoyment</c> and
    /// <c>get_Amount()</c>. <see cref="HandoverGoods"/> says the same from the
    /// other side: it skips any carried product whose <c>AppliedPackaging</c> is
    /// null, because that is not a thing a customer takes.
    /// </para>
    /// <para>
    /// <b>The smallest valid packaging, and the game's own list of them.</b>
    /// <c>ProductDefinition.ValidPackaging</c> is what the game allows for this
    /// product, and <c>PackagingDefinition.Quantity</c> is how many units go in
    /// one. The smallest is taken so that the unit count the caller asked about
    /// is expressible exactly rather than rounded up to the next brick — the
    /// probe is answering <i>what would they pay for these units</i>, and a
    /// bigger box would quietly change the question.
    /// </para>
    /// </remarks>
    private static void Package(ItemInstance instance, ProductDefinition product)
    {
        ProductItemInstance packaged = instance.TryCast<ProductItemInstance>();
        if (packaged is null || packaged.AppliedPackaging != null)
        {
            return;
        }

        PackagingDefinition smallest = SmallestPackaging(product);
        if (smallest != null)
        {
            packaged.SetPackaging(smallest);
        }
    }

    /// <summary>
    /// The valid packaging that holds the fewest units, or null when the game
    /// lists none for this product.
    /// </summary>
    private static PackagingDefinition SmallestPackaging(ProductDefinition product)
    {
        Il2CppInterop.Runtime.InteropTypes.Arrays.Il2CppReferenceArray<PackagingDefinition> valid =
            product.ValidPackaging;
        if (valid is null)
        {
            return null;
        }

        PackagingDefinition smallest = null;
        ...
        for (int i = 0; i < valid.Length; i++)
        {
            ...
            // A packaging that claims to hold nothing would make the offer
            // meaningless rather than small, so it is not a candidate.
            int held = candidate.Quantity;
            if (held <= 0 || held >= fewest)
            {
                continue;
            }
