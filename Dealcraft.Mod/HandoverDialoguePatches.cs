...
/// <c>ScheduleOne.Dialogue.DialogueController.Interacted()</c> (RVA
/// <c>0x6DCE70</c>, 509 bytes).</b> It is the method that opens the dialogue.
/// Read out of <c>GameAssembly.dll</c> with
/// <c>tools/native-truth/read-method.py</c>: at <c>0x1806dcf1c</c> it calls
/// <c>DialogueController.GetActiveChoices()</c> (RVA <c>0x6DC5E0</c>), at
/// <c>0x1806dcf30</c> it branches on that list's <c>Count</c>
...
/// at <c>0x1806dd005</c>. With no choices it takes the other side and plays a
/// worldspace one-liner instead (<c>5.0f</c> at <c>0x182dffb50</c>, virtual call
/// at <c>0x1806dcf69</c>) — no dialogue screen. So this method running and the
/// game deciding whether <i>Complete Deal</i> is on the prompt are one event,
/// and <c>GetActiveChoices</c> is the same method <c>docs/handover-truth.md</c>
/// already establishes as the one that decides it.
/// </para>
/// <para>
/// <b>It is reached by real calls and is not inlined.</b>
/// <c>DialogueController.Start()</c> (RVA <c>0x6DFB70</c>) calls
...
/// <c>DialogueController.StartGenericDialogue(Boolean)</c> (RVA <c>0x6DFB30</c>)
/// reaches <c>0x1806dce70</c> with a plain <c>call</c>. A detour at the entry
...
/// <c>0x1806afba8</c>) by the time <c>GetActiveChoices()</c> is called, so
...
internal static class HandoverDialoguePatches
{
    ...
    /// (RVA <c>0x6B7330</c>) does call <c>Component::GetComponent()</c>, which
    /// suggests <c>Customer</c> and <c>DialogueController</c> share a GameObject,
    /// but the generic argument is not named in the disassembly and nothing here
