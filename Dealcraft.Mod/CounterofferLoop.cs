...
/// the player presses Send — <c>Customer.CounterOfferClicked</c>
/// hands it to <c>CounterofferInterface.Open</c>, and <c>Send()</c> calls it with the screen's quantity and its price selector's
/// amount. Its body writes the player's reply into the
...
/// byte the RPC writer at, which has no other call site in the
...
internal sealed class CounterofferLoop
{
        ...
        in CounterofferLimits limits)
    {
        ...
        Negotiation advice = CounterofferSearch.For(
            ...
            // The offer on the table: the counter is judged against how good a
            // deal the customer already has, so their own figures go in with it.
            offered,
