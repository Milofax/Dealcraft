...
internal sealed class ListedPricePass
{
    ...
    private void Consider(ProductDefinition product, AdvisorSettings settings)
    {
        ...
        // A reading that stepped over anybody prices nothing. The price goes
        // into the player's save, the game builds every later offer from it, and
        // there is no undo — so the bar is that the whole roster answered, not
        // that enough of it did.
        if (!valuation.IsComplete)
        {
            string incomplete = valuation.Explaining(
                $"{valuation.ProductName}: its price is left at "
                + $"${valuation.AskingPrice:0.##} because the reading could not take every "
                + "customer, and a price worked out from some of them is the best price for a "
                + "roster you do not have");

            Say($"incomplete:{valuation.ProductId}", incomplete);
            DecisionRecord.ListedPrice(
                ListedPriceOutcome.CannotCompute, acted: false, incomplete, valuation.ProductId);
            return;
        }
        ...
        ListedPriceDecision decision = ListedPriceDecision.For(
            ...
            PricingRecommendation.Best(valuation.Candidates, valuation.MarketValue));
