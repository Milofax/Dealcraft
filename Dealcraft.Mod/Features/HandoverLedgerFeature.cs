...
internal sealed class HandoverLedgerFeature : Feature
{
    ...
    public override void Declare(MelonPreferences_Category category) =>
        _enabled = category.CreateEntry(
            // On where the mod is worked on, off in a released build, and a
            // player can still switch it on to send a bug report. See
            // Diagnostics: which build this is, is not a decision anybody makes
            // at release time.
            Key, Diagnostics.Recording,
            description: "Host only: write one line per handover to "
                ...
                + "bonuses it named. Off by default in a release. Nothing is installed while it is "
                + "off, and it never appears in the phone app: this is a measuring instrument, not "
                + "a feature. Read the file with bin/read-ledger.");
