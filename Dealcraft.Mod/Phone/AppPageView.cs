...
internal sealed class AppPageView
{
    ...
    public SceneLook Settle()
    {
        ...
        // An app that is shut has nothing with a size in it, and that is a
        // state of the scene rather than an answer. It used to be taken as one:
        // the page gave up for good at install time, when the app is always
        // shut, and the owner met the result every day — the settings running
        // up into the map's heading. The probe added for it is what settled it,
        // naming all four handles as found and the app as not on screen.
        if (frame != null && !frame.gameObject.activeInHierarchy)
        {
            Unsettled = "the app was shut when the page was measured, so the settings page may "
                + "sit over the header band until it is opened";
        ...
        if (frame == null || branches.Count == 0)
        {
            ...
            // Keyed on whether the app was on screen, not just on "no-frame".
            // The same key for both states meant a failure at install silenced
            // the far more interesting one with the app open — which is the
            // question the owner's overlap actually turns on.
            Say(
                "page-top",
                frame != null && frame.gameObject.activeInHierarchy ? "no-frame-open" : "no-frame-shut",
                "the app has no content area to measure, so the settings "
    ...
    /// of the machine code, — so the copy ends
