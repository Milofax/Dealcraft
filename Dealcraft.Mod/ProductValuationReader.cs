...
internal static class ProductValuationReader
{
    ...
    public static int OrderableQuantity(Customer customer, ProductDefinition product)
    {
        ...
        for (int i = 0; i < orderable.Count; i++)
        {
            ...
            {
                // m_Item2, the backing field, and not Item2, the property.
                // Il2CppInterop gives Tuple`2 both: Item2 calls the game's
                // generic get_Item2(), while m_Item2 reads the field through
                // NativeFieldInfoPtr_m_Item2. The generic call is the fragile
                // path when the type argument is a value type, and the owner's
                // session of 2026-09-18 is what caught it: every customer, on
                // every product, answered 2147483647 — int.MaxValue, not an
                // order size. The game's own cap is 1000, which it printed in
                // the same run, so the figure was never the game's.
                return entry.m_Item2;
            }
        }
