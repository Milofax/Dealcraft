...
internal static class ProductValuationReader
{
        ...
        AdvisorSettings settings)
    {
        ...
        for (int i = 0; i < customers.Count; i++)
        {
            ...
            if (!range.Searchable)
            {
                ...
                // Whether the customer has a dealer rides along, because the
                // game's own method branches on exactly that:
                // Customer.GetOrderableProductsWithQuantities (RVA 0x6A9790)
                // null-checks the dealer with UnityEngine.Object.op_Inequality
                // and only reaches Dealer.GetOrderableProducts(EQuality) on the
                // other side of it. Every customer answering the identical
                // figure is either a sentinel on the dealer-less path or a bad
                // read, and this is the one line that can tell those apart.
                valuation.Notes.Add(
                    $"{CustomerName(customer)} ({(customer.AssignedDealer == null ? "no dealer" : "has a dealer")})"
                    + $": {range.Refusal}");
                continue;
            }
    ...
    /// <c>0x3025a7d0</c> — the low half of a heap address, so it was reading the
