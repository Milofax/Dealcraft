...
public class ScenarioSixTests
{
    ...
    private const float Suggested = 41f;
    ...
    {
        TheScenarioFile.Says(Scenario, "Three of my fourteen customers go quiet at $52");
    ...
    {
        TheScenarioFile.Says(Scenario, "At $41 nobody is priced out.");
    ...
    /// </summary>
    [Fact]
    public void The_suggested_price_is_the_highest_at_which_nobody_goes_quiet()
    {
        TheScenarioFile.Says(Scenario, "`listed $52 · suggested $41 · priced out 3 of 14`");
        ...
        // Everybody, and that is the change: the ladder used to climb to $48
        // and twelve customers because a dearer rung took more money. It does
        // not — the price cancels out of what a customer pays — so the rung
        // that keeps the whole roster wins and the three who would have gone
        // quiet stay.
        Assert.Equal(14, best.CustomersReached);
        Assert.Equal(cheapest.PricePerUnit, best.PricePerUnit);
    }
    ...
    /// </summary>
    [Fact]
    public void Turned_on_it_writes_the_price_that_keeps_everybody()
    {
        PricingRecommendation best = PricingRecommendation.Best(Fourteen());
        ...
        Assert.Equal("OG Kush: $52 becomes $41, which 14 of your customers still clear ($5740 a week)", maintained.Reason);
    ...
    {
        TheScenarioFile.Says(Scenario, "Dealcraft reads it and says:");
    ...
    public void The_record_carries_both_prices_and_the_reason()
    {
        ...
        Assert.Contains("\"new_price\":41", row, StringComparison.Ordinal);
        Assert.Contains("14 of your customers still clear", row, StringComparison.Ordinal);
    }
}
