...
public class ScenarioFiveTests
{
    ...
    public void It_waits_and_takes_the_next_allowed_window_the_game_offers()
    {
        ...
        ScheduleDecision morning = DealScheduler.ChooseWindow(
            new ContractClaimRegistry().Claim(TheirContract),
            allowed,
        ...
        Assert.Equal(ScheduleOutcome.Schedule, morning.Outcome);
        Assert.Equal(DealWindow.Morning, morning.Window);
        Assert.Equal("Morning is the window the game is in now, and you allow it", morning.Reason);
    }
