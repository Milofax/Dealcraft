...
public class ExactlyTheGradeOrderedTests
{
    ...
    /// <summary>
    /// A fresh install may use a higher grade, which is the filled circle
    /// <c>app.md</c> draws in the HANDOVER block — on the host page and on the
    /// guest page both. The preferences file is seeded from this object, so this
    /// is the default in the file and in the app at once.
    /// </summary>
    /// <remarks>
    /// This asserted the opposite, from a user story that the drawing had
    /// already outgrown. The owner's session of 2026-09-18 is what decided it:
    /// with the strict default, every contract he had asked for Standard or Poor
    /// while his stock was Heavenly, so every handover was refused with <i>"it
    /// stays in the bag"</i> and the feature did nothing whatsoever. The strict
    /// rule is still there for whoever wants it; it is not what a player meets
    /// first.
    /// </remarks>
    [Fact]
    public void A_fresh_install_may_use_a_higher_grade()
    {
        Assert.True(new AdvisorSettings().MayUseAHigherGrade);
        Assert.Equal(GradeReach.MayUseAHigherGrade, new AdvisorSettings().Reach);
        Assert.Equal(
            GradeReach.Exactly,
            new AdvisorSettings { MayUseAHigherGrade = false }.Reach);
    }
