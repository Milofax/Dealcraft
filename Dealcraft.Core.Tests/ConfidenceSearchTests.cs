...
public class ConfidenceSearchTests
{
    ...
    /// <summary>
    /// And it says which price it took away, at what confidence, rather than
    /// reporting that price as one the customer would never take.
    /// </summary>
    /// <remarks>
    /// The floor filtering rungs out routed real offers into the refusal, which
    /// answered with the lowest rung's offer and a chance of zero bolted on. Six
    /// of the owner's negotiations on 2026-09-19 then read "countering at $286
    /// would be taken 0% of the time" about an offer the customer would have
    /// taken half the time. The figure the player needs is the one the floor
    /// cost them.
    /// </remarks>
    [Fact]
    public void A_refusal_names_the_offer_the_floor_took_away()
    {
        BestOffer best = ConfidenceSearch.Best(
            Curve((0.5f, 264f, 0.5f), (0.7f, 180f, 0.7f)),
            0.95f);
        ...
        Assert.False(best.Found);
        Assert.False(best.Offer.HasOffer);
        Assert.Contains("$264", best.Offer.Reason);
        Assert.Contains("50%", best.Offer.Reason);
        Assert.Contains("95%", best.Offer.Reason);
        Assert.DoesNotContain("0% of the time", best.Offer.Reason);
    }
