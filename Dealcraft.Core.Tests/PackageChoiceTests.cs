...
public class PackageChoiceTests
{
    ...
    {
        // Five slots of one baggie each, not one slot of twenty-five: the bound
        // is on the screen's positions and a position takes a whole stack.
        var bag = new[]
        {
            new CarriedLot(QualityTier.Standard, Baggie, 1),
            new CarriedLot(QualityTier.Standard, Baggie, 1),
        ...
        Assert.Equal(5, choice.Fill.Carrying);
    }
    ...
    public void No_option_holds_more_packages_than_a_handover()
    {
        ...
        {
            Assert.InRange(option.Positions, 1, PackagePlan.MostPositions);
        }
    }
}
