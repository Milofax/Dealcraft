...
public class PackagePlanTests
{
    ...
    /// <summary>
    /// Enough in the bag, but not in four packages — and four is what the
    /// handover screen holds. A player could not do it by hand either, so
    /// neither does the mod, and the row does not blame the stock.
    /// </summary>
    [Fact]
    public void Enough_product_in_too_many_packages_is_a_different_answer_from_too_little()
    {
        PackageFill fill = PackagePlan.Fill(new[] { new PackageLot(Baggie, 25) }, 20);

        Assert.False(fill.Covers);
        Assert.Equal(PackageFillOutcome.NeedsMorePackages, fill.Outcome);
        Assert.Equal(25, fill.Carrying);
    }
    ...
    public void Never_more_than_the_four_positions_the_handover_screen_holds()
    {
        ...
        {
            Assert.True(PackagePlan.Fill(bag, order).Packages <= PackagePlan.MostPackages);
        }
    }
    ...
    /// note on <see cref="PackagePlan.MostPackages"/>.
    /// </remarks>
    [Fact]
    public void It_is_never_worse_than_the_smallest_package_first_rule_it_replaced()
    {
        ...
        foreach (int bricks in new[] { 0, 1, 2 })
        {
            ...
            for (int order = 1; order <= 30; order++)
            {
                ...
                if (!fill.Covers || before < order || beforePackages > PackagePlan.MostPackages)
                {
                    continue;
                }
