...
public class PackagePlanTests
{
    ...
    /// <summary>
    /// A jar and a bag of baggies against an order of six: the jar goes, and
    /// nineteen baggies stay in the pocket.
    /// </summary>
    /// <remarks>
    /// The owner asked it as a question — <i>"wenn ich ein Glas habe und 20
    /// Tüten, und der andere möchte sechs haben, nimmst du dann wirklich das
    /// Glas und nicht alle Tüten?"</i> — and it is the whole of what the plan is
    /// for. Six baggies also cover the order exactly, out of one inventory slot
    /// and so one position. What decides it is the second rule: least overshoot
    /// first, then fewest packages. Both fills come to six units, and two
    /// packages beat six.
    /// </remarks>
    [Fact]
    public void A_jar_and_a_baggie_beat_six_baggies()
    {
        var bag = new[] { new PackageLot(Jar, 1), new PackageLot(Baggie, 20) };

        PackageFill fill = PackagePlan.Fill(bag, 6);
