...
public class AutomationFormTests
{
    ...
    /// path never reaches it. See <c>docs/counteroffer-truth.md</c>.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Story 23: exactly what was ordered is what a fresh install does, and it is
    /// the answer that is ticked rather than the absence of one.
    /// </summary>
    [Fact]
    public void Exactly_the_grade_that_was_ordered_is_the_answer_a_fresh_install_shows()
    {
        IReadOnlyList<FormRow> rows = Block(
            Form(new AdvisorSettings { AutoHandover = true }), "HANDOVER").Rows;

        Assert.True(rows[4].Chosen);
        Assert.False(rows[5].Chosen);
        ...
        Assert.Equal("On", Form(AllOn()).ValueOf("MayUseAHigherGrade"));
    }
