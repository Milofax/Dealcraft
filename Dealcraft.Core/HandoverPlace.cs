...
/// need one. <c>DialogueController.Interacted()</c> (RVA <c>0x6DCE70</c>) is the
/// method that opens it: it calls <c>GetActiveChoices()</c> (RVA
/// <c>0x6DC5E0</c>) at <c>0x1806dcf1c</c>, and reaches
/// <c>DialogueHandler.StartDialogue</c> at <c>0x1806dd005</c> only when that
/// list is non-empty (<c>cmp 0x18(%rax)</c>, <c>jg</c> at <c>0x1806dcf30</c>).
/// So the moment this value names is the same moment the game decides whether
/// <i>Complete Deal</i> is on the prompt. <c>docs/handover-truth.md</c> carries
...
/// <c>InteractionManager.CheckHover</c> (RVA <c>0x64ABB0</c>) against the
/// object's own <c>MaxInteractionRange</c> — and gated on it. That reading was
/// sound and is still in <c>docs/handover-truth.md</c>, but a distance was only
...
/// (<c>Contract.SubmitPayment</c>, RVA <c>0xA9B200</c>). A host on
...
/// <c>.scratch/deal-automation/app.md</c> records the decision.
/// </para>
/// </remarks>
