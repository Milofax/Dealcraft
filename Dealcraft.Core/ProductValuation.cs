...
public sealed class ProductValuation
{
    ...
    /// whose order size the game's own limits refuse, most of all. They are the
    /// only place the refused figure is ever written down, so they go to the log
    /// and to the debug record beside the decision.
    /// </summary>
    /// <remarks>
    /// This used to say these were "not failures of the reading — the other
    /// customers still answered". That sentence cost the owner his price list on
    /// 2026-09-19. Every customer was being refused for an unreadable order size
    /// and the pass carried on with whoever was left, recommending $2 for a
    /// product he listed at $160 and $7 for one at $280 — then, because the game
    /// builds its offers from the listed price, his customers started asking for
    /// forty-five units at a few hundred dollars. A reading that stepped over
    /// anybody is not a reading to price a product from. See
    /// <see cref="IsComplete"/>.
    /// </remarks>
    public List<string> Notes { get; } = new();

    /// <summary>
    /// Whether every customer who could be asked actually answered.
    /// </summary>
    /// <remarks>
    /// The gate for writing a price, and deliberately the strictest form of it:
    /// one customer the reading could not take is enough to hold the whole
    /// product. Prices are written into a save the player cannot undo, and the
    /// figure a partial reading produces is not a smaller truth but a different
    /// one — it is the best price for a roster that does not exist.
    /// </remarks>
    public bool IsComplete => Notes.Count == 0;
