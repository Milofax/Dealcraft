...
/// own: <c>ProductManagerApp.Start</c> (RVA <c>0x9EC220</c>) subscribes its own
/// <c>CreateEntry</c> to <c>ProductManager.onProductDiscovered</c> (the field at
/// <c>+0x120</c>, written at <c>0x1809ec339</c>–<c>0x1809ec3df</c> with a
/// delegate whose target is <c>this</c>), and
/// <c>ProductManagerLoader.Load</c> (RVA <c>0x58C780</c>) calls
/// <c>SetProductDiscovered</c> once per saved product. Its logic body (RVA
/// <c>0x7FEB50</c>) adds the product and invokes that event at
/// <c>0x1807fed4f</c>. So an app cloned while a save is loading watches the
...
/// (RVA <c>0x9EB300</c>, the append at <c>0x1809eb52a</c> through
