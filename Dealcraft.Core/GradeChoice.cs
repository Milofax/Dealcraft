...
public static class GradeChoice
{
    ...
    /// <see cref="PackagePlan.MostPackages"/> packages — five baggies of
        ...
        GradeReach reach)
    {
        ...
        return $"{QualityTier.Name(choice.Quality)} has {choice.Fill.Carrying} units in reach and no "
            + $"{PackagePlan.MostPackages} packages of it come to the "
            + $"{request.RequestedQuantity} the contract asks for — a handover holds "
            + $"{PackagePlan.MostPackages} packages, so this needs fewer, larger ones";
    }
