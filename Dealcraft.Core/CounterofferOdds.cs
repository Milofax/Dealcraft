...
public static class CounterofferOdds
{
    ...
    private static float Value(float marketValue, float pricePerUnit)
    {
        ...
        // Asking nothing is the best offer there is, and it has to say so.
        //
        // This answered zero, together with a guard in Of that refused a total
        // of zero outright. Both were meant against nonsense input and both made
        // the whole function non-monotone: the search bisects between its bounds
        // and takes a zero at each end as proof there is nothing between them.
        // The owner's session of 2026-09-19 shows the cost exactly - "first 3
        // for $1000000 answered 0, last 3 for $0 answered 0, best anywhere 0" -
        // for a customer who had offered $260 a unit against a listed price of
        // $250 and would have taken the counter for certain.
        if (pricePerUnit <= 0f)
        {
            return 2f;
        }
