...
/// lists every choice whose <c>ShouldShow()</c> is true and
/// filters on nothing else; <c>DialogueChoice.ShouldShow()</c> is <c>shouldShowCheck(Enabled)</c>; and
/// <c>Customer.SetUpDialogue</c> sets the <i>Complete
...
/// <c>DialogueController.CheckChoice</c> — is the condition
/// under which pressing it does anything. the project notes carries
/// the reading.
/// </para>
/// <para>
...
/// <c>DialogueController.Interacted()</c>, the method that
/// calls <c>GetActiveChoices()</c> and
/// <c>DialogueHandler.StartDialogue</c>. Pressing <c>E</c>
...
/// <c>InteractionManager.CheckHover</c> comparing
/// <c>Vector3.Distance(playerCamera.position, hitPoint)</c> against that
/// object's own <c>MaxInteractionRange</c> (<c>+0x30</c>, at
///–) — and gated on it. The reading stands
/// and stays in the project notes. The gate does not, because a
...
/// <c>Customer.IsAtDealLocation</c> is called from exactly
/// two places in the game, both inside
/// <c>DealerAttendDealBehaviour</c> — an NPC dealer delivering for you.
/// <c>Customer.IsDealTime</c> is called from exactly two,
...
/// was ours rather than the game's.</b> <c>Customer.ProcessHandover</c> reads <c>IsClientInitialized</c> and calls
