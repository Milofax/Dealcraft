...
- `Automated handover when I talk to them` hangs on the game's own method for
  opening a dialogue — `DialogueController.Interacted`, read out of
  `GameAssembly.dll` — so the mod holds no distance of its own at all. It used
  to hold one, and that was the only number in the feature the binary had not
  supplied. **Not yet seen on a screen:** the reading says the handover finishes
...
Dealcraft is a rebuild of the ideas in **Deal Optimizer** by *xyrilyn* and its
fork **High Baller** by *zocke1r*, both GPL-3. Dealcraft is GPL-3 as well. See
[`LICENSE`](LICENSE).

Neither original author is involved in this project and neither is responsible
for it.
